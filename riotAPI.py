...
class LeagueParsing:
    def makeView(master, participant, row, col):
        newFrame = customtkinter.CTkFrame(master=master)
        newFrame.grid(row=row, column=col, padx=3, pady=3)
        ...
        LeagueParsing.parseChamp(newFrame, participant)
    ...
    def display(masterFrame, summoner):
        ...
        for participant in summoner.current_match.blue_team.participants:
            if(row < 4):
                LeagueParsing.makeView(masterFrame, participant, row, col)
            else:
                LeagueParsing.makeView(masterFrame, participant, row-3, col+1)
            row += 1
        row = 1
        col = 3
        for participant in summoner.current_match.red_team.participants:
            if(row < 3):
                ...
                LeagueParsing.makeView(masterFrame, participant, row-2, col+1)
            row += 1
